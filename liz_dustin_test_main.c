...
/*                                                                            */
/*                                                        ::::::::            */
/*   liz_dustin_test_main.c                             :+:    :+:            */
/*                                                     +:+                    */
/*   By: epanholz <epanholz@student.codam.nl>         +#+                     */
/*                                                   +#+                      */
/*   Created: 2020/01/13 20:13:04 by epanholz       #+#    #+#                */
/*   Updated: 2020/02/27 17:45:31 by kstallen      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int     main(void)
{
	...
	printf("%s++++ Enable slow output? [Y]es, [n]o, [M]aybe, [I] don't know ++++%s\n", KRED, KNRM);
	scanf("%c", &c);

	if (c == 'm' || c == 'M')
	{
		c = 0;
		printf("Darling, you should be more decisive in your life!\n");
		printf("So once again: Slow output [Y]es or [n]o?\n");
		scanf(" %c", &c);
		printf("%sCongrats! You made a decision!\n", KRED);
		sleep(3);
		if (c == 'y' || c == 'Y')
		{
			printf("%s+++SLOW TESTS ENABLED+++%s\n", KBLU, KNRM);
			printf("+++TEST WILL START IN 100000+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99999+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99998+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99997+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99996+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99995+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99994+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99993+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99992+++\n");
			sleep(1);
			printf("+++TEST WILL START IN  99991+++\n");
			sleep(1);
			printf("+++%sJust kidding...here you go:%s+++\n", KRED, KNRM);
			sleep(3);
		}
	}
	else if (c == 'I' || c == 'i')
	{
		printf("%s++++++ Very good option dude ++++++%s\n", KBLU, KNRM);
		sleep(1);
		printf("%sMalcolm in the Middle mode enabled%s\n", KBLU, KNRM);
		sleep(1);
		printf("%sYes, no, maybe\n", KRED);
		sleep(2);
		printf("%si don't know\n", KRED);
		sleep(2);
		printf("%scan you repeat the question?\n", KRED);
		sleep(4);
		printf("%sYOU'RE NOT THE BOSS OF ME NOW!\n", KRED);
		sleep(1);
		printf("%sYOU'RE NOT THE BOSS OF ME NOW!\n", KRED);
		sleep(1);
		printf("%sYOU'RE NOT THE BOSS OF ME NOW!\n", KRED);
		sleep(1);
		printf("%sAnd you're not sooooooo biiig!\n", KRED);
		sleep(1);
		printf("%sOkay enough jokes for today let's test your ft_printf\n%s", KRED, KNRM);
		sleep(3);
		c = 'y';
	}
	...
	printf("%s---- TEST 39 ----%s\n", KRED, KNRM);
    printf("[%d]\n", printf("REAL [%4.104d]", 42));
	printf("[%d]\n", ft_printf("MINE [%4.104d]", 42));
	printf("\n");
	if (c == 'y' || c == 'Y')
		sleep(1);
	...
	printf("%s+++GO!+++%s\n", KRED, KNRM);
	sleep(2);
    ret_own =   ft_printf("|%-10.d%-.5d%dABC%%%24.12%%%00%%%5.4%%%%-.5%%%%020%%0.20%%020%%d%d%10d%02u%10.10u%-20.4u%-.u%5.u%c%c%c%c%4221c%s%s%s%s|\n", i0, i1, i2, i3, i4, i5, i6, ui0, ui1, ui2, ui3, ui4, c0, c1, c2, c3, s0, s1, s2, s4);
    ret_original = printf("|%-10.d%-.5d%dABC%%%24.12%%%00%%%5.4%%%%-.5%%%%020%%0.20%%020%%d%d%10d%02u%10.10u%-20.4u%-.u%5.u%c%c%c%c%4221c%s%s%s%s|\n", i0, i1, i2, i3, i4, i5, i6, ui0, ui1, ui2, ui3, ui4, c0, c1, c2, c3, s0, s1, s2, s4);
	printf("\n");
	if (c == 'y' || c == 'Y')
		sleep(1);
	if (ret_original == ret_own)
	{
		...
		sleep(1);
		printf("%sNow you are ready to try out the ultimate super final Boss: Moulinette!%s\n", KRED, KNRM);
	...
	// 103 - ("%2u", -1) : FAILED
	// 104 - ("%3u", -1) : FAILED
	// 105 - ("%9u", UINT_MAX) : FAILED
	// 106 - ("%10u", UINT_MAX) : FAILED
	// 107 - ("%11u", UINT_MAX) : FAILED
	// 159 - ("%-2u", -1) : FAILED
	// 160 - ("%-3u", -1) : FAILED
	// 161 - ("%-9u", UINT_MAX) : FAILED
	// 162 - ("%-10u", UINT_MAX) : FAILED
	// 163 - ("%-11u", UINT_MAX) : FAILED
	// 206 - ("%02u", -1) : FAILED
	// 207 - ("%03u", -1) : FAILED
	// 208 - ("%09u", UINT_MAX) : FAILED
	// 209 - ("%010u", UINT_MAX) : FAILED
	// 210 - ("%011u", UINT_MAX) : FAILED
	// 245 - ("%.s", 42) : FAILED
	// 308 - ("%.2u", -1) : FAILED
	// 309 - ("%.3u", -1) : FAILED
	// 312 - ("%.3u", -42) : FAILED
	// 313 - ("%.4u", -42) : FAILED
	// 314 - ("%.20u", UINT_MAX) : FAILED
	// 684 - ("%-9.0u", UINT_MAX) : FAILED
	// 685 - ("%-9.1u", UINT_MAX) : FAILED
	// 686 - ("%-9.2u", UINT_MAX) : FAILED
	// 687 - ("%-9.10u", UINT_MAX) : FAILED
	// 688 - ("%-10.0u", UINT_MAX) : FAILED
	// 689 - ("%-10.1u", UINT_MAX) : FAILED
	// 690 - ("%-10.2u", UINT_MAX) : FAILED
	// 691 - ("%-10.10u", UINT_MAX) : FAILED
	// 692 - ("%-11.0u", UINT_MAX) : FAILED
	// 693 - ("%-11.1u", UINT_MAX) : FAILED
	// 694 - ("%-11.2u", UINT_MAX) : FAILED
	// 695 - ("%-11.10u", UINT_MAX) : FAILED
	// ft_printf("ultimate1 %*d        %*i\n", 500, 0, 10, 10);
